...
class Display(IObserver):
    ...
    def selectMap(self, maps_data: list):
        """Display map selection menu populated from the database."""
        options = [
            (m['map_type'], m['display_name'], m['description'])
            for m in maps_data
        ]
        idx    = self._menuSelect("  SELECT YOUR MAP  ", options)
        chosen = maps_data[idx]
        return chosen['map_type'], chosen['default_size'], chosen['default_nb_monsters']

    def selectHero(self, heroes_data: list):
        """Display hero selection menu populated from the database."""
        options = [
            (h['id'], h['name'], h['description'])
            for h in heroes_data
        ]
        idx    = self._menuSelect("  SELECT YOUR RACE  ", options)
        chosen = heroes_data[idx]

...
class AbstractMap(IObservable, ABC):
    def __init__(self, size, nbMonster, display, heroData, heroName, empty_token=' . '):
        super().__init__()
        self._display = display
        self._heroData = heroData
        self._heroName = heroName
        self.__size = size
        self.__empty_token = empty_token
        self.__characters = []
    ...
    def _initPlayer(self):
        return HeroFactory.create(self._heroData, self._heroName, self._getValidCoord())

    def _createMonster(self, coord):
        return MonsterFactory.create(coord)

    def _emptyToken(self):
        return self.__empty_token
    ...
    def __fight(self, monster: AbstractMonster):
        ...
        if not hero.isDead():
            ...
            leather = monster.getLeather()
            loot = []
            if gold > 0:
                loot.append(f"{gold} gold")
            if leather > 0:
                loot.append(f"{leather} leather")
            hero.loot(monster)
            hero.rest()
            hero.killCount += 1
